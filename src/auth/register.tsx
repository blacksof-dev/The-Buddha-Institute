...
const Register = () => {
  ...
  const handleOtp = async () => {
    try {
      ...
      if (response.data.status) {
        ...
        setLoadingOtp(false);
        toast.error(response.data.message || "Some error during sending otp");
      }
    } catch (error: any) {
      console.error("API Error:", error.response?.data || error.message);
  ...
  const verifyOtp = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.REACT_APP_APPLICATION_URL}/api/verify_otp`,
        { mobile: mobileNumber, otp: otpValue },
        { headers: { "Content-Type": "application/json" } }
      );
    ...
    <section className="mt-24 xl:mt-44 px-4 blade-bottom-margin-sm flex justify-center items-center">
      <div className="w-full max-w-3xl bg-white p-2 md:p-8 shadow-xl border-2 border-customGray/10 rounded-2xl">
        ...
        <form
          ...
          className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 p-2 md:p-4"
        >
          {/* Name */}
          <div>
            <label className="block font-lato-regular text-black mb-1 text-sm md:text-base">
              Name
            </label>
            <input
              type="text"
          ...
          <div className="md:col-span-2 flex items-center space-x-2 mt-2">
            ...
            <label
              htmlFor="terms"
              className="font-lato-regular text-black text-sm md:text-base"
            >
