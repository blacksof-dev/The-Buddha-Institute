...
export default function AwardsAdminPanel() {
  const [data, setData] = useState<dataProps[]>();
  const [isPopupOpen, setIsPopupOpen] = useState<boolean>(false);
  const [awardsId, setAwardsId] = useState<string>("");
  ...
  const handleDeleteFromServer = async (id: string) => {
    try {
      const res = await axios.delete(
        `${process.env.REACT_APP_LOCAL_URL}/api/awards/delete/${id}`
      );
  ...
  const handleFormSubmitEdit = async (formData: any) => {
    ...
    try {
      await axios.request({
        method: "PATCH",
        url: `${process.env.REACT_APP_LOCAL_URL}/api/awards/edit/${formData._id}`,
        data: formData,
        headers: {
  ...
  const handleFormSubmitCreate = async (formData: any) => {
    try {
      await axios.request({
        method: "POST",
        url: `${process.env.REACT_APP_LOCAL_URL}/api/awards/create`,
        data: formData,
        headers: {
          "Content-Type": "multipart/form-data",
        },
          ...
          <div className=" border-b-[1px] w-full pb-4 flex justify-between">
            ...
            <div>
              ...
              >
                ...
                <h4 className="text-nowrap"> Add new Award</h4>
              </button>
            </div>
          </div>
        </div>
                  ...
                  <div className="bg-white  h-full flex flex-col justify-between">
                    <div>
                      ...
                      <div className="p-4 ">
                        ...
                        <h6 className="font-lato-regular text-black/80 mb-4">
                          {obj.desc}
                        </h6>
