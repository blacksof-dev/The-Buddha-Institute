...
export default function GovtInstitutions() {
        ...
        const fetchContactDetails = async () => {
            try {
                const response = await axios.get(
                    `${process.env.REACT_APP_LOCAL_URL}/api/GovtInstitue/show`
                );
    ...
    const handleDeleteFromServer = async (id: string) => {
        try {
            const res = await axios.delete(`${process.env.REACT_APP_LOCAL_URL}/api/GovtInstitue/delete/${id}`);
