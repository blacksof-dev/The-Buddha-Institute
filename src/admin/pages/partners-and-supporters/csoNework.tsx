...
export default function CsoNework() {
        ...
        const fetchContactDetails = async () => {
            try {
                const response = await axios.get(
                    `${process.env.REACT_APP_LOCAL_URL}/api/csonetwork/show`
                );
    ...
    const handleDeleteFromServer = async (id: string) => {
        try {
            const res = await axios.delete(`${process.env.REACT_APP_LOCAL_URL}/api/csonetwork/delete/${id}`);
        ...
        <div className='flex flex-wrap gap-8 pt-8'>
            {
                data && data.map((object: logoProps, index) => (
                    <div className='w-[20rem] d  p-4 flex flex-col  rounded-md justify-between  border-[1px] border-gray-200' key={index}>
                        ...
                        <div className='mt-4 font-lato-regular'>
                            <h6 className='text-black/80'>{object.title}</h6>
                        </div>
