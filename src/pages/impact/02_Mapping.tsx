...
const Mapping = () => {
  ...
  const fetchDetails = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_LOCAL_URL}/api/mapDetails/show`);
          ...
          <div className="flex lg:flex-row flex-col gap-4 lg:gap-40 blade-top-padding-sm blade-bottom-padding-sm">
            <div className="w-full lg:w-[50%]">
              <h4 className="font-lato-regular text-black/80 ">
                ...
                India’s grassroots to empowerment at scale through their
                diverse, innovative enterprises.
              </h4>
            </div>

            <div className="w-full  lg:w-[50%]">
              <h4 className="font-lato-regular text-black/80 ">
                Rooted in compassion, they see rural India and its citizens as
                the “centers for value addition”. By generating stable
                employment opportunities and livelihoods right at ground zero,
                these Development Entrepreneurs are enabling dignified living
                while reducing rural migration across India.
              </h4>
            </div>
          </div>
                ...
                <div className="grid  lg:gap-5 lg:grid-cols-4 sm:grid-cols-3 grid-cols-2 ">
                  {mapapidata?.map((elem, index) => {
                    ...
                    return (
                      <div
                        key={index}
                        className={`border-b border-[#84848480]  p-4    ${isFullWidthItem ? "col-span-2 sm:col-span-1" : ""
                          }  ${index === 0 || index === 4
