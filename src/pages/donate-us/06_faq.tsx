...
function Accordion({
    ...
    if (accordionContent) {
      if (isOpen) {
        gsap.to(accordionContent, {
          ...
          duration: 0.5,
          ease: "power1.out",
          // onComplete: () => console.log("Accordion opened successfully")
        });
      } else {
        gsap.to(accordionContent, {
          height: 0,
          duration: 0.5,
          ease: "power1.in",
          // onComplete: () => console.log("Accordion closed successfully")
        });
      }
    }
  }, [isOpen]);
  ...
  return (
    <div className="bg-[#FAF9F5] md:py-6 py-2 justify-center accordion-item overflow-hidden border-b border-[#7F7F7F]">
      <button
        type="button"
        className="accordion-header  flex transition-colors duration-300 ease-in-out  gap-3 group outline-none rounded-md pr-2 w-full"
        onClick={toggleAccordion}
      >
        <h4 className=" flex-1  2xl:text-2xl font-lato-bold leading-tight text-left">
          {title}
        </h4>
        <div className="2xl:mt-1.5 xl:mt-1 mt-1.5 grid relative place-content-start 2xl:ml-40 rounded-full">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            className={`md:w-5 h-4 md:h-5 transition-transform duration-300 ease-in-out ${
          ...
          <svg
            className={`absolute inset-0 fill-[#1a8482] md:w-5 h-4 md:h-5 transition-all duration-300 ease-in-out ${
              isOpen ? "opacity-100 rotate-180" : "opacity-0 -rotate-180"
            }`}
            viewBox="0 0 448 512"
            xmlns="http://www.w3.org/2000/svg"
          >
